...
custom_style = Style(
    [
        ...
        (
            "highlighted",
            "fg:#000000 bold bg:#ffff00 underline",
        ),  # Highlighted option - bold black on yellow with underline
...
def show_welcome(is_first_run: bool):  # pragma: no cover
...
[/bold cyan]"""
    ...
    if is_first_run:
        console.print("[yellow]Welcome! This is your first run.[/yellow]")
        console.print("[green]Configuration file created: .dbml_to_sqlmodel[/green]")
    else:
        console.print(
            "[bold white]Generate SQLModel models and FastAPI CRUD from DBML schemas[/bold white]"
        )

    console.print()


def run_initial_setup():  # pragma: no cover
    """Run first-time setup to populate settings."""
    config = config_manager.config

    console.print()
    console.print("[bold cyan]Initial Setup[/bold cyan]")
    console.print(
...
def show_menu_header():  # pragma: no cover
    """Show just the menu header without logo."""
    console.print()
    console.print("[bold cyan]DBML to SQLModel Generator[/bold cyan]")
...
def interactive_menu():  # pragma: no cover
    ...
    while True:
        # Clear screen before showing menu (except first time)
        ...
        choice = questionary.select(
            ...
            choices=[
                questionary.Choice(
                    "[>] Settings            - Configure generator options", value="settings"
                ),
                questionary.Choice(
                    "[>] DBML → Code         - Generate FastAPI from schema", value="generate"
                ),
                questionary.Choice(
                    "[>] Code → DBML         - Extract schema from models", value="code_to_dbml"
                ),
                questionary.Choice(
                    "[>] Report              - View generation statistics", value="report"
                ),
        ...
        if choice is None or choice == "exit":
            console.clear()
            console.print()
            console.print("[bold cyan]Thank you for using DBML to SQLModel![/bold cyan]")
            console.print("[dim]Build amazing APIs![/dim]")
            console.print()
            break

        # Clear screen and show compact header for results
        console.clear()
        console.print("[bold cyan]DBML to SQLModel[/bold cyan]")
...
def handle_settings():  # pragma: no cover
    ...
    while True:
        ...
        table.add_row("Show new file contents", "Yes" if config.show_new_content else "No")
        table.add_row("Overwrite protected files", "Yes" if config.force_overwrite else "No")
        table.add_row("Require SQLAdmin login", "Yes" if config.admin_auth_enabled else "No")
        ...
        action = questionary.select(
            ...
            choices=[
                ...
                questionary.Choice("Toggle: show new file contents", value="show_new_content"),
                questionary.Choice("Toggle: overwrite protected files", value="force_overwrite"),
                questionary.Choice("Toggle: SQLAdmin login", value="admin_auth_enabled"),
...
def handle_report():  # pragma: no cover
...
def handle_generate():  # pragma: no cover
    ...
    if output_dir.exists():
        ...
        changes_count = preview_summary.get("new", 0) + preview_summary.get("modified", 0)
        if changes_count > 0:
...
def handle_code_to_dbml():  # pragma: no cover
