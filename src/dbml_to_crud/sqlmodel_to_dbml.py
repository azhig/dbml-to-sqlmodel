...
from .dbml_to_sqlmodel import ColumnInfo, TableInfo, parse_dbml
...
def _parse_model_file(model_file: Path, enum_map: dict[str, str]) -> TableInfo:
    ...
    for class_def in class_map.values():
        if any(kw.arg == "table" and isinstance(kw.value, ast.Constant) and kw.value.value is True for kw in class_def.keywords):
...
def apply_dbml_table_updates(existing_dbml: str, generated_dbml: str) -> str:
    ...
    normalized_existing_blocks = {
        name: "\n".join(table_block_lines(table))
        for name, table in existing_table_map.items()
    }
    normalized_generated_blocks = {
        name: "\n".join(table_block_lines(table))
        for name, table in generated_table_map.items()
    }
