...
from dbml_to_sqlmodel import generate_sqlmodel, parse_dbml, parse_dbml_enums
...
def generate_admin_views(tables):
    imports = "\n".join([f"from models.{table.name} import {table.name.capitalize()}" for table in tables])

    code = f"""from sqladmin import Admin, ModelView

{imports}

# Иконки по умолчанию для разных типов таблиц
DEFAULT_ICONS = {{
...
def get_icon_for_table(table_name):
    \"\"\"Подбор иконки на основе имени таблицы\"\"\"
...
"""
    for table in tables:
        ...
        # Добавляем column_labels с описаниями из note
...
def main():
    ...
    with open(os.path.join(output_dir, "admin.py"), "w") as f:
        f.write(generate_admin_views(tables))
