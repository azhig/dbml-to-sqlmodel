...
"""
    ...
    # Словарь для хранения relationships по таблицам
    table_relationships = {}
    ...
    # Сначала собираем все relationships
    for table in tables:
        ...
        for column in table.columns:
            if column.references:
                ...
                if target_table != table.name or target_col != column.name:
                    ...
                    rel_name = target_table.rstrip("s")  # Убираем 's' в конце
    ...
    for table in tables:
        ...
        for column in table.columns:
            ...
            if column.references:
                ...
                if target_table and target_col:
                    # Добавляем default=None для nullable FK
                    if column.nullable:
                        field_args.insert(0, "default=None")
                    field_args.append(f"foreign_key='{target_table}.{target_col}'")

            # В цикле генерации полей, после создания field_args:
            if column.note:
        ...
        if table.name in table_relationships:
            class_def.append("")  # Пустая строка для читаемости
    ...
    for table in tables:
        ...
        # Добавляем docstring для самого класса
        if any(c.note for c in table.columns):
            create_class_def.append(f'    """Схема для создания {table.name}"""')
    ...
    for table in tables:
        ...
        # Добавляем docstring для самого класса
        if any(c.note for c in table.columns):
            update_class_def.append(f'    """Схема для обновления {table.name}"""')
